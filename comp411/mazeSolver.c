...
char maze[100][100];             // 100x100 is the maximum size needed
  ...
  */
  ...
  for(y=0; y < height; y++) {
    for(x=0; x < width; x++) {
      ...
      wasHere[y][x] = false;
      correctPath[y][x] = tempchar;
    }
    scanf("\n");    // This is used to "eat" the newline
  }

  bool b = recursiveSolve(startX, startY);
  correctPath[startY][startX] = 'S';
  correctPath[endY][endX] = 'F';
           // Code to print the output maze
  for (y = 0; y < height; y++) {
      for (x = 0; x < width; x++) {
          printf("%c", correctPath[y][x]);
          if (x == width - 1) {
              printf("\n");
          }
      }
  ...
  */
    ...
    if (x != 0) // Checks if not on left edge
        if (recursiveSolve(x-1, y)) { // Calls method one to the left
            correctPath[y][x] = '.'; // Sets that path value to true;
            return true;
        }
